...
class Tuner(object):
    ...
    def add_attr(self, aname, rp, slave_index, **extra):
        extra['format'] = '%7.4f'
        extra['display level'] = 'Tg.DispLevel.EXPERT'
        ...
        def write_f(inst, wattr):
            ...
            write_value = wattr.get_write_value(data)
	...
	if rp.rw == Tg.READ_WRITE:
	  try:
	      write_attr = self.impl._attr(aname)
	      value = self.impl._read(aname).value
	      write_attr.set_write_value(value)
	      self.impl.log.info('attribute %r added %r',aname, value)
	  except Tg.DevFailed, fail:
	      self.impl.log.info('attribute %r added, failed reading', aname)
